  ...
  return (
    <span className="flex gap-1.25">
      {keys.map((k) => (
        <a
          key={k}
          className="flex size-7  items-center justify-center rounded-[3px] border border-line bg-panel2 transition-[border-color,transform] duration-150 hover:-translate-y-px hover:border-dim"
          href={links[k]}
          target="_blank"
        ...
        >
          <img
            src={PLATFORM_ICONS[k]}
            alt={PLATFORM_NAMES[k]}
...
const typeChip =
  "rounded-xs border border-line2 px-1.5 py-0.5 font-mono text-[9.5px] tracking-[0.08em] text-dim uppercase";
...
function DiscoRow({
  ...
  d: DiscoEntry;
  onOpen: (d: DiscoEntry) => void;
}) {
  return (
    <div className="grid grid-cols-[64px_1fr_auto_auto] items-center gap-3 rounded-sm border border-line bg-panel py-2 pr-3 pl-2 transition-[border-color] duration-150 hover:border-line2">
      <button
      ...
      >
        <img
          className="size-16  object-cover transition-transform duration-200 group-hover:scale-[1.06]"
          src={d.art}
          alt={d.title}
          loading="lazy"
        />
  ...
  return (
    ...
    >
      <button
        ...
        onClick={onClose}
      />
      <div className="relative max-h-[90vh] w-full max-w-105 overflow-y-auto rounded-lg border border-line2 bg-panel motion-safe:animate-[fade-in_.2s_ease]">
        <button
          className="absolute top-2 right-2 z-1 flex size-8  items-center justify-center rounded-sm border border-line2 bg-panel/80 text-dim backdrop-blur-sm transition-colors hover:border-dim hover:text-daw-text"
          aria-label="close"
          onClick={onClose}
        >
          ✕
        ...
        <div className="flex flex-col gap-3 px-5 py-4">
          <div className="flex items-start justify-between gap-3">
            <h3 className="m-0 text-[18px] leading-tight font-bold text-pretty">
              {d.title}
            </h3>
            <span className={typeChip + " shrink-0"}>{d.type}</span>
          </div>
          {d.note ? (
            <p className="m-0 text-[13px] leading-[1.6] text-pretty text-daw-text">
              {d.note}
            </p>
          ) : null}
          <PlatformLinks links={d.links} />
...
export function DiscoGroup({
  ...
  return (
    <div>
      <div className="mb-2 font-mono text-[11px] tracking-widest text-faint uppercase">
        {label} · {items.length}
      </div>

...
const { values: f } = parseArgs({
  options: {
    ...
    trimStart: { type: "string" }, // drop the first N frames off the export head; overrides cfg.trimStart
  },
});
...
// before the tour actually starts off the front in ffmpeg. (skill step 5b)
...
await revealBgVideo();
// Some sites load the bg <video>/<img> via a post-mount fetch, so it's a black/
// blank fallback for a while after `networkidle`. If cfg.waitBg is set, wait until
// that element has actually PAINTED before recording, so the tour never runs
// against the fallback. See skill step 5b.
if (cfg.waitBg)
  await page
    .waitForFunction(
      (sel) => {
...
const clickNav = async (text, waitText) => {
  await page.getByRole("link", { name: text, exact: true }).first().click();
  // lazy route? wait for distinctive target text to PAINT before dwelling, else
  // the recording shows the Suspense fallback (blank) while the chunk loads.
  if (waitText) await page.getByText(waitText).first().waitFor({ state: "visible" });
  await revealBgVideo();
};
...
// (5b), UNLESS cfg.keepIntro: then the blank-load + entrance animation are the
// intended opening, so keep the whole head.
// trimStart: drop this many extra frames off the export head (flag overrides
// config), converted to seconds via --fps. Stacks on top of the head-trim, so it
// works whether keepIntro is on or off.
const trimStart = Number(f.trimStart ?? cfg.trimStart ?? 0);
const headTrim = cfg.keepIntro ? 0 : Math.max(0, (Date.now() - recordStart) / 1000 - 0.3); // keep 0.3s lead-in
const trimSec = headTrim + trimStart / Number(f.fps);
...
for (const beat of cfg.beats ?? []) {
  if (beat.visit !== undefined) {
    ...
    await clickNav(beat.clickNav, beat.waitText);
    await hold(beat.dwell);
    ...
    await hover(beat.hover);
    await hold(beat.dwell);
    console.log("hover", beat.hover);
...
// keyframe seek; applied to every encode below). 0 when nothing to trim. (5b)
const trim = trimSec > 0.1 ? ["-ss", trimSec.toFixed(2)] : [];
if (trim.length)
  console.log(`trimming ${trimSec.toFixed(2)}s off head` + (trimStart ? ` (incl. trimStart ${trimStart}f)` : ""));
